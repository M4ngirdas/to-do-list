...
export default function Menu(props) {
    ...
    const listElements = props.lists.map(list => (
        <li onClick={() => { props.openList(list.id); props.setIsMobileMenuOpen(false) }} key={list.id} className={`${props.currentList?.id === list.id ? "bg-slate-800/50" : ""} group flex items-center p-2 w-full h-11 md:h-12 cursor-pointer rounded-sm overflow-hidden transition-transform duration-200 active:scale-95 border border-slate-700/50 hover:bg-slate-800/30`}>
            <div className="flex gap-2 w-full overflow-hidden">
                <p className="truncate">{list.title}</p>
            </div>
            <div className="grid place-items-center">
                <button onClick={(ev) => { ev.stopPropagation(); props.showConfirmation("list", list.id) }} className="grid md:opacity-0 place-items-center h-7 w-7 rounded-sm transition-opacity duration-200 hover:bg-rose-700 group-hover:opacity-100"><FaTimes /></button>
            </div>
        </li>
    ))
        ...
        <>
            ...
            <>
                <nav className={`${isMenuOpen ? "hidden md:grid" : "hidden"} transition-transform duration-200 content-start gap-6 relative p-6 w-80 rounded-md`} >
                    ...
                    <button onClick={props.showSettings} className="flex items-center w-full h-11 md:h-12 p-2 gap-2 font-semibold rounded-sm transition-transform duration-200 active:scale-95 border border-slate-700/50 bg-slate-800/60 hover:bg-slate-700/50"><span className="px-2"><FaPlus /></span>Create new list</button>
            ...
            <nav className={`${props.isMobileMenuOpen ? "translate-y-0 p-4" : "-translate-y-[96%] p-0"} flex md:hidden flex-col gap-4 fixed inset-4 transition-all duration-200 rounded-sm border border-slate-700/50 bg-gray-950`}>
                <img className="w-full h-22 object-contain select-none scale-325 -z-1" src="./images/logo.png" alt="Tasked to-do list app logo" />
                <button onClick={props.showSettings} className="flex items-center gap-2 p-2 h-11 md:h-12 font-semibold rounded-sm border border-slate-700/50 bg-slate-800/60 hover:bg-slate-700/50"><span className="px-2"><FaPlus /></span>Create new list</button>
                <p className="font-medium text-slate-500">MY LISTS</p>
